...
#define ServoPin0 9        // номер вывода для подключения сервопривода галетника
#define ServoPin1 10       // номер вывода для подключения сервопривода конденсатора 1
#define ServoPin2 11       // номер вывода для подключения сервопривода конденсатора 2

#define MinVarCap 2        // начальный угол сервопривода конденсаторов
#define MaxVarCap 170      // конечный угол сервопривода конденсаторов
#define StepVarCap 25      // шаг изменения угла сервопривода конденсаторов

#define DetenSelector 1    // угол ослабления натяга сервопривода галетника

#define ServoSlow 50       // Задержка (мс) при повороте сервопривода на 1 градус
...
#define NumBands 5         // количество диапазонов
...
byte BandPin[NumBands] = {15,16,17,18,19}; // номера входов на которые подаётся сигнал от декодера диапазонов
                                        // цифровые 0-13, аналоговые 14-19 
                                        // Заняты: 13 цифр. - светодиод, 19 аналоговый - клавиши
...
byte SelectorAngle[NumBands] = {2,30,60,20,160}; // углы поворота галетника для каждого диапазона в соотв. с порядком в BandPin[]
...
boolean BandChanged(void)
{
  ...
  if (NewBand == Band)
    ...
    {
     if (SelectorAngle[Band] < SelectorAngle[NewBand]) // если угол поворота галетника в сторону увеличения, то ослаблять натяг в противоположную
       {DetenDir=-1;}
     else 
       {DetenDir=1;}     
     Band=NewBand; 
...
void SlowMove(Servo Srv, byte nA)
{
  byte n;
  byte s;
  n=Srv.read();
  ...
  if (BandChanged())
    {
      ...
      if (Band < NumBands)
      {
        ...
        // установить сервы
        //SlowMove(Servo0, SelectorAngle[Band]);        
        SlowMove(Servo0, pServo0);        
        
        Serial.print("Selector angle set - ");
        Serial.print(pServo0);
        Serial.print("\n");
/*
        Servo0.write(SelectorAngle[Band]+DetenSelector*DetenDir); // ослабление галетника

        Serial.print("Angle deten - ");
        Serial.print(SelectorAngle[Band]+DetenSelector*DetenDir);
        Serial.print("\n");
*/        
        SlowMove(Servo1, pServo1);        
        SlowMove(Servo2, pServo2);
        
        if (!ServoAttached)   // подключаемся к сервоприводам только после того, как установили угол, иначе, при включении ардуино они встанут в случайные позиции
        {
          ServoAttached=true;
